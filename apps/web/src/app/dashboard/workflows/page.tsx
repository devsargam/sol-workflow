...
import { Plus, Pencil, Trash2, Workflow as WorkflowIcon, Pause, Play, Loader2 } from "lucide-react";
    ...
    <div className="flex flex-1 flex-col gap-6 p-6">
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {workflows.map((w) => (
            <Card key={w.id} className="flex flex-col transition hover:border-foreground/20">
                ...
                <div className="flex shrink-0 items-center gap-1.5 rounded-full border bg-background px-2.5 py-1">
                  <span
                    className={cn(
                      ...
                      w.enabled ? "bg-[#14F195] shadow-[0_0_6px_#14F195]" : "bg-muted-foreground"
                    )}
                  />
                  <span className="text-xs font-medium">{w.enabled ? "Active" : "Paused"}</span>
                </div>
              </CardHeader>
              <CardContent className="mt-auto flex items-center justify-between gap-2 border-t pt-4">
                {(() => {
                  const isToggling = toggleMutation.isPending && toggleMutation.variables === w.id;
                  return (
                    <Button
                      size="sm"
                      variant="outline"
      ...
      {pendingDelete ? (
        <DeleteModal
          ...
          description="This will permanently delete the workflow."
